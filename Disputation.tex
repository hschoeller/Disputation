...
\definecolor{predcol}{RGB}{150,150,150}
\definecolor{obscol}{RGB}{60,160,110}
\definecolor{updcol}{RGB}{70,70,70}

% x0, mu, sigma, amplitude, style
\newcommand{\dens}[5]{%
  \draw[#5] plot[domain={#2-3.0*#3}:{#2+3.0*#3}, samples=50,
    variable=\y]
    ({#1+#4*exp(-((\y-#2)*(\y-#2))/(2*#3*#3))},\y) -- cycle;}
...
% WHY: teach the DA cycle and its three uncertainty sources in
%   one picture, for a room where not everyone is a DA person.
% SAY: "A forecast needs an initial state, but we never observe
%   the atmosphere fully. Data assimilation combines what we do
%   observe with a short-range forecast -- the background -- to
%   estimate it. Three things make that estimate uncertain: the
%   observations carry error and don't cover everything, and the
%   model that propagated the background has error of its own.
%   ERA5 quantifies this by running the whole cycle ten times
%   with perturbed observations. The spread of that ensemble is
%   my object of study."
\begin{frame}{Forecasts start from an uncertain state}
  \centering
  \begin{tikzpicture}[
      box/.style={draw, rounded corners, align=center,
                  font=\small, minimum height=.85cm,
                  minimum width=2.4cm},
      src/.style={align=center, font=\scriptsize,
                  text=gray!60!black},
      arr/.style={-{Stealth[length=2mm]}, thick}]

    \node[box] (obs) at (-2.6,1.4) {Observations};
    \node[box] (bg)  at ( 2.6,1.4) {Background};
    \node[box] (ana) at ( 0,-0.4) {Analysis};

    \node[src] (obsunc) at (-2.6,3.0)
      {uncertainty $\cdot$ coverage};
    \node[src] (mdlunc) at ( 2.6,3.0) {model error};
    \draw[gray, dashed] (obsunc) -- (obs);
    \draw[gray, dashed] (mdlunc) -- (bg);

    \draw[arr] (obs) -- (ana);
    \draw[arr] (bg)  -- (ana);
    \draw[arr] (ana.east) to[bend right=55]
      node[right, font=\scriptsize, align=left, xshift=2pt]
      {short-range\\ forecast} (bg.south);

    \coordinate (fan) at (0,-1.4);
    \draw[arr] (ana) -- (fan);
    \foreach \a/\o in {28/0.35, 14/0.65, 0/1, -14/0.65, -28/0.35}{
      \draw[gray, opacity=\o, thick] (fan) -- ++({270+\a}:1.5);
    }
    \node[src, anchor=north west] at (1.0,-2.2)
      {ensemble spread};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Forecasts start from an uncertain state}
  \centering
  \begin{tikzpicture}[x=1.15cm, y=1.05cm, font=\small,
      pred/.style={draw=predcol, fill=predcol!18},
      obs/.style={draw=obscol!80!black, fill=obscol, opacity=.75},
      upd/.style={draw=updcol, fill=updcol!75, opacity=.85}]

    \draw[-{Stealth[length=2mm]}, gray] (-0.4,1.4) -- (8.9,1.4);
    \foreach \t/\lab in {0/$t_0$, 1.6/$t_1$, 3.2/$t_2$,
                         4.8/$t_3$, 6.4/$t_4$, 8.0/$t_5$}{
      \node[font=\scriptsize, anchor=north] at (\t,1.35) {\lab};
    }

    % --- 1: model predictions ---
    \dens{0}{2.50}{0.28}{0.43}{pred}
    \dens{1.6}{3.05}{0.45}{0.27}{pred}
    \dens{3.2}{3.25}{0.45}{0.27}{pred}
    \dens{4.8}{3.45}{0.45}{0.27}{pred}
    \dens{6.4}{3.65}{0.50}{0.24}{pred}
    \dens{8.0}{3.95}{0.58}{0.21}{pred}

    % --- 2: observations ---
    \only<2->{
      \dens{1.6}{2.62}{0.24}{0.50}{obs}
      \dens{3.2}{2.85}{0.24}{0.50}{obs}
      \dens{4.8}{3.05}{0.24}{0.50}{obs}
    }

    % --- 3: analyses ---
    \only<3->{
      \dens{1.6}{2.72}{0.20}{0.60}{upd}
      \dens{3.2}{2.95}{0.20}{0.60}{upd}
      \dens{4.8}{3.15}{0.20}{0.60}{upd}
      \draw[obscol, dashed, thick] plot[smooth]
        coordinates {(0,2.55) (1.6,2.75) (3.2,2.95) (4.8,3.15)
                     (6.4,3.35) (8.0,3.55)};
    }

    % --- 4: coverage ---
    \only<4>{
      \draw[gray!50!black, thick, dotted]
        (6.4,2.6) ellipse (0.55 and 1.5);
      \node[anchor=north, align=center, font=\scriptsize,
            text=gray!50!black] at (6.6,1.05)
        {no observations:\\ uncertainty not reduced};
    }
  \end{tikzpicture}
\end{frame}
